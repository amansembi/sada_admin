...
if(isset($_REQUEST['id'])){
	$bill=$_REQUEST['id'];
$run=mysqli_query($con,"select * from `bill` where bid='$bill'");
if($run)
{
...
<table>
<tr>
<td>Recpit No. : <?php echo $res['id'];?>/-</td>
...
$q ="select * from `bill` where bid='$bill'";
...
$t = round($r+$g, 3);
$sr++;
$fees=mysqli_query($con,"SELECT SUM(amount) AS `total_quantity` FROM `bill` WHERE bid='$bill'");
$fees = mysqli_fetch_array($fees);
$total_qut = $fees['total_quantity'];
$fees=mysqli_query($con,"SELECT SUM(amount/100*18) AS `total_gst` FROM `bill` WHERE bid='$bill'");
$fees = mysqli_fetch_array($fees);
